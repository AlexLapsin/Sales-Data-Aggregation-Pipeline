import pandas as pd
import pytest
from transform import parse_dates, clean_basic, cap_extremes, derive_fields, rename_columns

# 1) Test date parsing
def test_parse_dates():
    df = pd.DataFrame({'Order Date':['7/27/2012','invalid'], 'Ship Date':['07/28/2012','7/29/2012']})
    out = parse_dates(df)
...
def test_clean_basic():
    df = pd.DataFrame({
        'Quantity':[1,0,-1], 'Sales':[100,200,300],
...
def test_rename_columns():
    df = pd.DataFrame({
        'Region':['X'], 'Country':['Y'], 'Order ID':['1'],
        'Order Date':[pd.Timestamp('2025-01-01')], 'Ship Date':[pd.Timestamp('2025-01-02')],
        'Customer ID':['C'], 'Product ID':['P'], 'Category':['Cat'],
        'Quantity':[1], 'Sales':[100], 'Profit':[10]
    })
    out = rename_columns(df)
    expected = ['region','country','order_id','order_date','ship_date',
                'customer_id','product_id','category','quantity','total_sales','profit']
