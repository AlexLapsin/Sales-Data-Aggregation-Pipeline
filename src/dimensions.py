...
sales_df = pd.read_sql_query(
    "SELECT * FROM sales_daily", conn,
    parse_dates=["order_date"]
)
conn.close()

# 2) Build dim_date table
# Extract unique order dates and compute date parts
...
# Extract unique product-level attributes
...
# Join sales_df with dims to assemble fact records
...
# 5) Write each table to Parquet using f-strings with OUTPUT_DIR
for name, df in [
    ("dim_date", dim_date),
