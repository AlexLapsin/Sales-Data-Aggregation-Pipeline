...
warnings.warn(
    "Legacy PostgreSQL pipeline is deprecated. "
    "Please migrate to cloud_sales_pipeline_dag.py. "
    "See legacy/README.md for migration guide.",
    DeprecationWarning,
    stacklevel=2,
)
...
with DAG(
    dag_id="legacy_postgres_sales_aggregation",
    description="⚠️ DEPRECATED: Legacy PostgreSQL Sales ETL - Use cloud_sales_pipeline_dag.py instead",
    default_args=default_args,
    schedule_interval=None,  # Disabled by default to prevent accidental runs
    start_date=datetime(2025, 7, 25),
    catchup=False,
    tags=["legacy", "deprecated", "postgres", "etl", "sales"],
    doc_md="""
    # ⚠️ DEPRECATED DAG ⚠️
    ...
    ## Migration Required
    Please switch to the modern cloud-native pipeline:
    - **New DAG**: `cloud_sales_pipeline_dag.py`
    - **Migration Guide**: See `legacy/README.md`
    - **Support**: Limited to security fixes only

    ## Why Deprecated?
    - Replaced by Snowflake/Databricks integration
    ...
    preflight_check = DockerOperator(
        ...
        api_version="auto",
        auto_remove=True,
        command="python -m legacy.orchestration.scripts.postgres_preflight_check",
        working_dir="/app/src",
    ...
    stage_raw_to_s3 = DockerOperator(
        ...
        api_version="auto",
        auto_remove=True,
        command="python -m legacy.orchestration.scripts.postgres_upload_data",
        working_dir="/app/src",
        environment={
            **ENV_VARS,
    ...
    transform = DockerOperator(
        ...
        api_version="auto",
        auto_remove=True,
        command="python -m legacy.orchestration.scripts.postgres_transform",
        working_dir="/app/src",
        environment={
            **ENV_VARS,
            "S3_PREFIX": "runs/{{ ds }}/",
    ...
    create_tables = DockerOperator(
        ...
        api_version="auto",
        auto_remove=True,
        command="python -m legacy.orchestration.scripts.postgres_create_tables",
        working_dir="/app/src",
        environment=ENV_VARS,
        docker_url="unix://var/run/docker.sock",
        network_mode="bridge",
    ...
    load = DockerOperator(
        ...
        api_version="auto",
        auto_remove=True,
        command="python -m legacy.orchestration.scripts.postgres_load",
        working_dir="/app/src",
        environment={
            **ENV_VARS,
            "S3_PREFIX": "runs/{{ ds }}/",
